...
- `Source/DragonOath/ItemSystem/Equipment/DOEquipmentAppearanceRegistry.h/.cpp`
...
本轮实施严格按 Phase 1 -> Phase 2 -> Phase 3 -> Phase 4 -> Phase 5 顺序推进，未把后续阶段的职责提前塞回前一阶段。

| 阶段 | C++ 状态 | 编辑器/资产状态 | 自动化/网络状态 |
|---|---|---|---|
| Phase 1 基础 Item | 已完成：核心实例类型、Definition Resolver、真实 SaveGame 字段、Typed 属性、QuickBar 零库存恢复 | 已生成 `背包装备系统优化_蓝图待办.md` | `DragonOath.Inventory` 11/11 通过；PIE 网络仍需按待办执行 |
| Phase 2 Inventory | 已完成：FastArray 删除前回调、聚合通知、事务 Revision、OperationResult、Pending 对账和复杂消耗品闭环；本轮接入 SortConfig 与跨组件 Mutation | 已生成 `背包装备系统优化_Phase2_蓝图待办.md` | `DragonOath.Inventory` 11/11 通过；网络 PIE 待执行 |
| Phase 3 Equipment/GAS | 已完成主体：EquipmentInstance、来源化 AbilitySet、GE/GA/Tag 精确撤销、恢复回滚；本轮接入 Affix、耐久运行时状态、EquipmentLayout/CompatibleSlotQuery | 已生成 `背包装备系统优化_Phase3_蓝图待办.md` | `DragonOath.Equipment` 7/7 通过 |
| Phase 4 网络同步 | 已完成主体：Owner-only 私有状态、Pawn 公开外观 FastArray、PresentationChanged、外观注册查询、重生重建；本轮补齐复制契约测试 | 已生成 `背包装备系统优化_Phase4_蓝图待办.md`；仍需创建并配置 `DOEquipmentAppearanceRegistry` | `DragonOath.ItemSystem.Network` 2/2 通过；仍需 Listen Server/观察客户端和延迟模拟 |
| Phase 5 UI | 已完成主体：稳定子 ViewModel、Tooltip VM、拖放域、异步资源过期校验、QuickBar 槽位缓存；本轮改为布局驱动装备槽并区分结构刷新/内容重绘 | 已生成 `背包装备系统优化_Phase5_蓝图待办.md` | DragonOath 模块已编译链接；仍需编辑器绑定检查和 UI 网络回归 |

### 15.1 已验证事实

- `DragonOath.Inventory` 当前命令行执行结果为 11/11 通过，包含 Phase 2 的排序、批量 Mutation 和真实 SaveGame Archive 往返。
- `DragonOath.Equipment` 当前命令行执行结果为 7/7 通过，包含 `AffixSpec`、`DurabilityRuntimeState`、`AppearanceRegistry`、`SaveAndRestore`、`SnapshotValidation`、`Transaction` 和 `SourceGrantLifecycle`。
- `DragonOath.Equipment.AppearanceRegistry` 在 `-DDC-ForceMemoryCache` 下命令行执行结果为 1/1 通过，验证了精确变体解析、空变体回退和未知外观拒绝。
- `SourceGrantLifecycle` 使用具体测试 Ability 和无属性无限 GE，验证了 AbilitySpec/GameplayEffect 的来源绑定与精确撤销。
- `DOInventorySlateWidgets.cpp` 已补齐 `DOInventoryComponent.h` 依赖；本次 DragonOath 模块源码完成编译和链接。
- SaveGame、FastArray、OperationResult、ViewModel 缓存和公开外观均未改变 VRM4U 源码或配置。

### 15.2 当前构建限制

完整 `DragonOathEditor` 构建当前同时受到工程既有插件问题影响：`Plugins/VRM4U` 存在 Unity 重复符号问题，`Plugins/UnLua`/`UnLuaExtensions` 在 UE 5.8 工具链下也有已有 API/警告级错误升级为编译错误。本次按要求没有修改这些插件，也没有将它们的问题归因到 DragonOath 物品系统。

项目模块的现有二进制已支持上述命令行自动化测试；完整 Editor 构建当前仍被 VRM4U Unity 重复符号问题阻塞。本次未修改 VRM4U 或 UnLua；该阻塞不影响已通过的 Item/Equipment/Appearance Registry 自动化结果，也不影响阶段代码和蓝图待办落盘。

### 15.3 最终验收顺序

1. 等待外部插件官方更新或可编译配置，重新编译完整 DragonOathEditor。
2. 在编辑器完成 Phase 3-5 蓝图待办中的资产绑定、Appearance 注册表和 Widget 配置。
3. PIE Listen Server + 1 Client 验证 Phase 2 OperationResult/Pending 和删除刷新。
4. 增加观察客户端验证 Phase 4 公开外观、私有数据边界、重生和延迟时序。
5. 回归 Tooltip、DragDrop、异步资源和 QuickBar 零库存显示。

### 15.4 本轮自动化报告

- Inventory 报告：本轮日志 `Saved/Logs/Codex_Phase23_Inventory_Rerun.log`，`DragonOath.Inventory` 共 11 项，全部 Success。
- Equipment 报告：本轮日志 `Saved/Logs/Codex_Phase23_Equipment_Rerun.log`，`DragonOath.Equipment` 共 7 项，全部 Success。
- Network 报告：本轮日志 `Saved/Logs/Codex_Phase23_Network_Rerun2.log`，`DragonOath.ItemSystem.Network` 共 2 项，全部 Success。
- 命令行测试使用 `-DDC-ForceMemoryCache` 绕过本机 Installed DDC 无可写节点问题；该参数只影响测试启动缓存，不修改项目配置。
- 以上报告不替代 Listen Server/观察客户端 PIE、Appearance Registry 资产绑定和 Widget 资产绑定验收；这些仍按 Phase 4/5 蓝图待办执行。

### 15.7 编译时“内存指向错误”说明

历史日志中出现过两类容易被误认为同一问题的错误：

1. `CoreNet.h:340 Condition == Other.Condition`：这是旧网络契约测试伪造复制条件时触发的 UE5.8 断言，不是背包运行时指针悬空。测试已改为通过 `FRepLayout::CreateFromClass` 查询真实复制条件，最新 Network 2/2 通过且无该断言。
2. `DerivedDataBackends.cpp:813 Unable to use cache graph 'Installed'`：这是本机 Installed DDC 没有可写节点，编辑器启动阶段会弹错误。自动化/验证命令使用 `-DDC-ForceMemoryCache` 绕过；它不修改项目数据。若本地编辑器仍弹出，应修复 DDC 路径权限或临时使用该参数。

完整 Editor 构建时还可能加载工程既有 VRM4U Unity 重复符号问题；本项目按约定未修改 VRM4U，建议使用 `-DisablePlugin=VRM4U` 验证 DragonOath 模块，避免把外部插件崩溃误判为本次背包代码错误。

### 15.5 本轮顺序实施补充

本轮按 Phase 2 -> Phase 3 -> Phase 4 -> Phase 5 顺序继续实施，未停留在 Phase 1：

- Phase 2：`UDOInventorySortConfig` 已真正接入排序；`BeginMutation/EndMutation` 合并跨组件事务广播；新增 `SortConfig` 与 `MutationRevisionBatch` 自动化测试。
- Phase 3：装备词缀进入属性 Spec；耐久归零撤销属性和装备 Ability、修复后重建；新增可配置 `UDOEquipmentLayout` 和 `CompatibleSlotQuery`；新增 Affix/Durability 自动化测试。
- Phase 4：新增 `DOItemSystemNetworkTests.cpp`，反射校验 Owner-only 完整状态、公开 Presentation 状态及私有字段边界。
- Phase 5：装备槽从 Layout/组件支持的槽位集合生成；ViewModel 区分列表结构变化与单格内容变化，避免每次数量变化都要求 TileView 重建。

本轮新增测试源码和 UHT/编译验证完成后，才将对应阶段标记为已验证。完整 Editor 构建若仍在 VRM4U `VRM4ULoader` Unity 重复定义处失败，只记录为外部阻塞，不修改 VRM4U 或 UnLua。

### 15.6 本轮最新自动化验收（2026-08-06）

- `DragonOath.Inventory`：11/11 Success。覆盖 `SortConfig`、`MutationRevisionBatch`、真实 SaveGame Archive 往返及 QuickBar 零库存恢复。
- `DragonOath.Equipment`：7/7 Success。覆盖 `AffixSpec`、`DurabilityRuntimeState`、`AppearanceRegistry`、`SaveAndRestore`、`SnapshotValidation`、`SourceGrantLifecycle`、`Transaction`。
- `DragonOath.ItemSystem.Network`：2/2 Success。`PublicAppearanceBoundary` 与 `ReplicationContract` 均通过；此前由测试伪造组件导致的 UE5.8 `CoreNet.h:340` 断言已修复，测试现在通过 `FRepLayout::CreateFromClass` 查询最终复制条件。
- 最新 DragonOath 模块已通过 UE5.8 Development Editor 编译并部署到 `Binaries/Win64/UnrealEditor-DragonOath.dll`。构建使用工作区 UBA 临时根目录并显式禁用 VRM4U；未修改 VRM4U/UnLua。
- 仍未完成的验收仅包括编辑器内资产绑定、Listen Server/Owner Client/观察客户端 PIE、延迟与重生回归，以及 UI Widget 的实际绑定；这些属于蓝图待办，不应标记为 C++ 已完成。
