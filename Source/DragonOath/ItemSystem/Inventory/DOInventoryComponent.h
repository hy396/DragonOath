...
public:
	...
	bool TryUseItemByInstanceId(const FGuid& InstanceId, EDOInventoryFailureReason& OutFailureReason);

	/** 快捷栏按物品定义查找并使用一个消耗品。 */
	bool TryUseItemByDefinition(const FPrimaryAssetId& DefinitionId, EDOInventoryFailureReason& OutFailureReason);

	/** 复杂道具 Ability/Event 在最终提交点调用的服务器接口。 */
	bool CommitConsumableUse(const FGuid& InstanceId, const FPrimaryAssetId& ExpectedDefinitionId, EDOInventoryFailureReason& OutFailureReason);
...
private:
	...
	bool TryUseItemInternal(const FDOItemInstanceRecord& Item, EDOInventoryFailureReason& OutFailureReason);
	...
	bool BeginComplexConsumableUse(const FDOItemInstanceRecord& Item, const UDOItemFragment_Consumable& Fragment, EDOInventoryFailureReason& OutFailureReason);
	...
	void BroadcastChanged(const TArray<FGuid>& ChangedInstanceIds);
