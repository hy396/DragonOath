...
#include "DOGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DOGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EDOEditorLoadMode
{
...
namespace DOGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("DO.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via DOGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UDOGameplayCueManager::DumpGameplayCues));

	static EDOEditorLoadMode LoadMode = EDOEditorLoadMode::LoadUpfront;
}
...
UDOGameplayCueManager::UDOGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UDOGameplayCueManager* UDOGameplayCueManager::Get()
{
	return Cast<UDOGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UDOGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UDOGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogDragonOath, Warning, TEXT("UDOGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UDOGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (DOGameplayCueManagerCvars::LoadMode)
	{
	case EDOEditorLoadMode::LoadUpfront:
		return true;
	case EDOEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EDOEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UDOGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UDOGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UDOGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UDOGameplayCueManager* GCM = Cast<UDOGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogDragonOath, Error, TEXT("DumpGameplayCues failed. No UDOGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogDragonOath, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogDragonOath, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogDragonOath, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogDragonOath, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogDragonOath, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogDragonOath, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogDragonOath, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogDragonOath, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogDragonOath, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogDragonOath, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogDragonOath, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogDragonOath, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UDOGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UDOGameplayCueManager* StrongThis = Get())
					{
...
void UDOGameplayCueManager::HandlePostGarbageCollect()
{
...
void UDOGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogDragonOath, Warning, TEXT("UDOGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UDOGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (DOGameplayCueManagerCvars::LoadMode)
	{
	case EDOEditorLoadMode::LoadUpfront:
...
void UDOGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UDOGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UDOGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UDOGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UDOGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UDOGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
