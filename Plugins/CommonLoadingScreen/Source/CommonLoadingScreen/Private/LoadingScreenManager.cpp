...
UWorld* ULoadingScreenManager::GetTickableGameObjectWorld() const
{
	...
	{
		return LocalGameInstance->GetWorld();
	}
...
bool ULoadingScreenManager::CheckForAnyNeedToShowLoadingScreen()
{
	...
	{
		DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("The game instance is null"));
		return true;
	}
	...
	{
		DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("The game viewport client is null"));
		return true;
	}
...
void ULoadingScreenManager::ShowLoadingScreen()
{
	...
	else
	{
		...
		if (LocalGameInstance == nullptr)
		{
			UE_LOG(LogLoadingScreen, Warning, TEXT("Unable to show loading screen because the game instance is null."));
		...
		if (GameViewportClient == nullptr)
		{
			UE_LOG(LogLoadingScreen, Warning, TEXT("Unable to show loading screen because the game viewport client is null."));
			bCurrentlyShowingLoadingScreen = false;
			return;
		}
...
{
	UGameInstance* LocalGameInstance = GetGameInstance();
	if (LocalGameInstance != nullptr && LoadingScreenWidget.IsValid())
	{
		if (UGameViewportClient* GameViewportClient = LocalGameInstance->GetGameViewportClient())
		{
...
void ULoadingScreenManager::ChangePerformanceSettings(bool bEnabingLoadingScreen)
{
	...
	if (GameViewportClient == nullptr)
	{
		UE_LOG(LogLoadingScreen, Warning, TEXT("Unable to change loading screen viewport settings because the game viewport client is null."));
		return;
	}
