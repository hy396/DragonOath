...
ULyraSettingsLocal* ULyraSettingsLocal::Get()
{
	...
	ULyraSettingsLocal* LyraSettings = Cast<ULyraSettingsLocal>(GameUserSettings);
	if (!LyraSettings && GameUserSettings)
	{
		UE_LOG(LogLyra, Warning, TEXT("LyraSettingsLocal::Get expected ULyraSettingsLocal but found %s. Configure GameUserSettingsClassName=/Script/Setly.LyraSettingsLocal."),
			*GetNameSafe(GameUserSettings->GetClass()));
	}
	return LyraSettings;
}
...
void ULyraSettingsLocal::ResetKeybindingsToDefault(ULyraLocalPlayer* LocalPlayer)
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
	{
		...
		{
			FString ProfileId;
			if (const UEnhancedPlayerMappableKeyProfile* CurrentProfile = UserSettings->GetCurrentKeyProfile())
			{
				ProfileId = CurrentProfile->GetProfileIdString();
			}
