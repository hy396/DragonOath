...
/** 创建/主持游戏会话的请求对象——存储 HostSession 所需的参数 */
...
/**
 * 本地用户请求加入会话事件——例如从平台邀请/Overlay 触发
 * 游戏通常应将玩家转入该会话
 * @param LocalPlatformUserId  接受邀请的本地平台用户 ID（可能尚未登录）
 * @param RequestedSession     请求的会话，处理出错时为 null
...
/**
 * 加入会话完成事件——底层会话加入后、跳转服务器前触发
 * @param Result 加入结果（成功/失败）
 */
