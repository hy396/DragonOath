...
#include "LexStaticMesh.generated.h"

USTRUCT(BlueprintType)
struct FLexUIStaticMeshVertex
{
	GENERATED_BODY()

	FLexUIStaticMeshVertex()
		:
...
/** Cache StaticMesh for use in LexUI's LexStaticMesh. Since we cannot read StaticMesh data in runtime, we must create this object and assign 'MeshAsset' property in editor. */
UCLASS(BlueprintType)
class LGUI_API ULexUIStaticMeshCacheData : public UObject
{
	GENERATED_BODY()

public:
	/** Access the slate vertexes. */
	const TArray<FLexUIStaticMeshVertex>& GetVertexData() const;
	...
	DECLARE_EVENT(ULexUIStaticMeshCacheData, FLexUIStaticMeshDataChangeEvent);
	FLexUIStaticMeshDataChangeEvent OnMeshDataChange;
#endif
private:
	// ~ UObject Interface
	...
	/** @see GetVertexData() */
	UPROPERTY()
		TArray<FLexUIStaticMeshVertex> VertexData;
...
enum class ELexStaticMeshVertexColorType :uint8
{
...
class LGUI_API ULexStaticMesh : public ULexVisualDirectMesh
{
	GENERATED_BODY()
